...
export interface EditorRecord { record_id: string; record_type: string; displayed_name: string; ownership: "campaign"; status: string; authority: "preparation" | "canon" | "revealed"; visibility: EditorVisibility; fields: EditorField[]; sections: EditorSection[]; connections: EditorConnection[]; content_digest: string; }
...
export interface EditorCreationContext { contract_name: "editor_creation_context"; contract_version: 1; campaign_id: string; viewed_revision: RevisionRef; head_revision: RevisionRef; editor_workflow_version: number; adapter_definition: AdapterDefinitionWire; }
...
export type EditorContext = EditorRecordView | EditorCreationContext;
export interface EditorFinding { finding_id: string; code: string; severity: "error" | "warning"; location: string; message?: string; recovery_action?: string; retryable: boolean; }
...
const recordDigestInput = (record: EditorRecord) => ({ record_id: record.record_id, record_type: record.record_type, displayed_name: record.displayed_name, ownership: record.ownership, status: record.status, authority: record.authority, visibility: record.visibility, fields: record.fields, sections: record.sections.map((section) => ({ ...section, body: section.body.replaceAll("\r\n", "\n").replaceAll("\r", "\n") })), connections: record.connections });
...
type ResponseError = Error & { category?: string; code?: string; findings?: EditorFinding[]; responseReceived?: boolean; retryable?: boolean; status?: number };
const request = async <T>(path: string, init: RequestInit = {}): Promise<T> => { const headers = new Headers(init.headers); headers.set("Accept", "application/json"); if (init.body !== undefined) headers.set("Content-Type", "application/json"); if (init.method && init.method !== "GET" && csrfToken) headers.set("X-CSRF-Token", csrfToken); const response = await fetch(`/api/v1${path}`, { ...init, credentials: "same-origin", headers }); csrfToken = response.headers.get("X-CSRF-Token") ?? csrfToken; const body = await response.json() as T & { error?: { code?: string; category?: string; findings?: EditorFinding[]; retryable?: boolean } }; if (!response.ok) { const code = body?.error?.code ?? "request_failed"; const error = new Error(code) as ResponseError; error.code = code; error.category = body?.error?.category; error.findings = Array.isArray(body?.error?.findings) ? body.error.findings : undefined; error.retryable = body?.error?.retryable; error.status = response.status; error.responseReceived = true; throw error; } return body; };
...
export const httpEditorApi = {
  ...
  creationContext: (campaignId: string, revisionIdValue: string) => request<EditorCreationContext>(`/campaigns/${encodeURIComponent(campaignId)}/revisions/${encodeURIComponent(revisionIdValue)}/editor/creation-context`),
