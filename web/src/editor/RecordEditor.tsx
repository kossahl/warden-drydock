...
export function RecordEditor({ campaignId, revisionId, recordId, proposalId, proposalVersion, navigate }: { campaignId: string; revisionId: string; recordId: string; proposalId?: string | null; proposalVersion?: number | null; navigate?: (href: string) => void }) {
  ...
  return <section className="card editor" aria-labelledby="editor-heading">
    ...
    {error && <div className="error editor-error" role="alert" aria-labelledby="editor-error-heading"><h3 id="editor-error-heading" ref={errorHeading} tabIndex={-1}>Editor error</h3><p>{error}</p>{validationFindings.length > 0 && <ul aria-label="Validation findings">{validationFindings.map((finding) => <li key={finding.finding_id} id={`${findingDescriptionId(finding)}-summary`}><strong>{finding.location}</strong>: {findingText(finding)}</li>)}</ul>}</div>}
