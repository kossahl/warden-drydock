...
import { httpEditorApi, nextConnectionId, type EditorConnection, type EditorContext, type EditorField, type EditorFinding, type EditorProposal, type EditorProposalReference, type EditorRecord, type EditorRecordView, type EditorRemovalImpact, type EditorSection, type RevisionRef } from "./editorClient";
...
const findingLocationControl = (location: string, record: EditorRecord): string | null => {
  ...
  if (["record_id", "record_type", "displayed_name", "status", "visibility"].includes(normalized)) return normalized;
  const fieldMatch = /^(?:field|fields)\.([^.]+)(?:\.value)?$/.exec(normalized);
  if (fieldMatch) {
    const field = /^\d+$/.test(fieldMatch[1]) ? record.fields[Number(fieldMatch[1])]?.field_id : fieldMatch[1];
    return field ? `field-${field}` : null;
  }
  const sectionMatch = /^(?:section|sections)\.([^.]+)(?:\.body)?$/.exec(normalized);
  if (sectionMatch) {
    const section = /^\d+$/.test(sectionMatch[1]) ? record.sections[Number(sectionMatch[1])]?.section_id : sectionMatch[1];
    return section ? `section-${section}` : null;
  }
  const connectionMatch = /^connections?\.([^.]+)(?:\.(target|target_record_id|relationship|state|context|connection_context))?$/.exec(normalized);
  if (!connectionMatch) return null;
  const connection = /^\d+$/.test(connectionMatch[1])
    ? record.connections[Number(connectionMatch[1])]
    : record.connections.find((item) => item.connection_id === connectionMatch[1]);
  if (!connection) return null;
  if (!connectionMatch[2]) return `connection-${connection.connection_id}`;
  const property = connectionMatch[2] === "target" || connectionMatch[2] === "target_record_id"
    ? "target"
    : connectionMatch[2] === "context" || connectionMatch[2] === "connection_context" ? "context" : connectionMatch[2];
  return `connection-${connection.connection_id}-${property}`;
};
const findingsByControl = (findings: EditorFinding[], record: EditorRecord): Record<string, EditorFinding[]> => findings.reduce<Record<string, EditorFinding[]>>((grouped, finding) => {
  const control = findingLocationControl(finding.location, record);
...
export function RecordEditor({ campaignId, revisionId, recordId, proposalId, proposalVersion, navigate }: { campaignId: string; revisionId: string; recordId: string; proposalId?: string | null; proposalVersion?: number | null; navigate?: (href: string) => void }) {
  ...
  const [validationFindings, setValidationFindings] = useState<EditorFinding[]>([]);
  const [validationControlFindings, setValidationControlFindings] = useState<Record<string, EditorFinding[]>>({});
  ...
  const load = (sourceRevisionId = revisionId) => {
    ...
    setBusy(false); setError(""); setValidationFindings([]); setValidationControlFindings({}); setMessage(""); setConflict(false); setImpact(null); setMode("edit"); setCorrectionMode(false); setCorrectionParentRevision(null); correctionDraft.current = null; correctionResolutions.current = null; correctionView.current = null; correctionImpact.current = null; correctionBase.current = null;
  ...
  const setValidationError = (reason: unknown) => {
    const findings = errorFindings(reason);
    setValidationFindings(findings);
    setValidationControlFindings(draft ? findingsByControl(findings, draft) : {});
  };
  const validate = (rejectNoop = false) => {
    ...
    if (definition?.requiredFields.includes("name") !== false && !draft.displayed_name.trim()) next.displayed_name = "Displayed name is required.";
  ...
  const startRemove = async () => {
    ...
    setBusy(true); setError(""); setMessage(""); setConflict(false); setValidationFindings([]); setValidationControlFindings({});
    try {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!isCurrentRequest()) return;
      setConflict(isStaleReason(reason)); setValidationError(reason); focusEditorError.current = true; setError(`Proposal was not created (${errorText(reason)}).`);
    } finally { if (isCurrentRequest()) setBusy(false); }
  };
  ...
  const startCorrection = async () => {
    ...
    setBusy(true); setFieldErrors({}); setValidationFindings([]); setValidationControlFindings({}); setError(""); setConflict(false);
    try {
  ...
  const cancelCorrection = () => {
    ...
    setFieldErrors({}); setValidationFindings([]); setValidationControlFindings({}); setCorrectionMode(false); setError(""); setConflict(false); setMessage("Correction canceled. The original proposal remains under review.");
  };
  const submitCorrection = async () => {
    ...
    const isCurrentRequest = () => correctionRequest.current === request.sequence
      && editorIdentityRef.current === request.editorIdentity
      && proposalIdentityRef.current === request.proposalIdentity;
    setBusy(true); setError(""); setConflict(false); setValidationFindings([]); setValidationControlFindings({});
    try {
      ...
      if (!isCurrentRequest()) return;
      setConflict(isStaleReason(reason)); setValidationError(reason); focusEditorError.current = true; setError(`Correction blocked (${errorText(reason)}). Reload the current head and rebase the fields.`);
    }
    finally { if (isCurrentRequest()) setBusy(false); }
  };
  const openCurrentHead = async () => {
    ...
    try {
      ...
      const current = campaigns.campaigns.find((item) => item.campaign_id === campaignId);
  ...
  const findingsFor = (key: string) => validationControlFindings[key] ?? [];
  const controlInvalid = (key: string) => !!invalid(key) || findingsFor(key).length > 0;
  const controlDescribedBy = (key: string, errorId: string) => describedBy(invalid(key) ? errorId : undefined, findingIds(findingsFor(key)));
  ...
  return <section className="card editor" aria-labelledby="editor-heading">
    {approvalDialogView}
    ...
    <fieldset disabled={fieldsLocked}><legend>Record details</legend>
      <label htmlFor="editor-record-id">Record ID</label><input id="editor-record-id" value={draft.record_id} readOnly={!isCreate || (!!proposal && correctionMode)} onChange={(event) => update({ record_id: event.target.value })} aria-invalid={controlInvalid("record_id")} aria-describedby={controlDescribedBy("record_id", "editor-record-id-error")} />{invalid("record_id") && <span id="editor-record-id-error" className="error">{invalid("record_id")}</span>}<ValidationFindingMessages findings={findingsFor("record_id")} />
      <label htmlFor="editor-name">Displayed name</label><input id="editor-name" value={draft.displayed_name} onChange={(event) => update({ displayed_name: event.target.value })} aria-invalid={controlInvalid("displayed_name")} aria-describedby={controlDescribedBy("displayed_name", "editor-name-error")} />{invalid("displayed_name") && <span id="editor-name-error" className="error">{invalid("displayed_name")}</span>}<ValidationFindingMessages findings={findingsFor("displayed_name")} />
      <label htmlFor="editor-type">Record type</label><select id="editor-type" value={draft.record_type} disabled={!isCreate || !!proposal} onChange={(event) => setDraft(newAdapterRecord(event.target.value, draft.record_id, draft.displayed_name, definitions.recordDefinitions))} aria-invalid={controlInvalid("record_type")} aria-describedby={controlDescribedBy("record_type", "editor-type-error")}>{(isCreate ? definitions.recordTypes : [draft.record_type]).map((type) => <option key={type} value={type}>{type}</option>)}</select>{invalid("record_type") && <span id="editor-type-error" className="error" role="alert">{invalid("record_type")}</span>}<ValidationFindingMessages findings={findingsFor("record_type")} />
      <label htmlFor="editor-status">Status</label><select id="editor-status" value={draft.status} onChange={(event) => update({ status: event.target.value, authority: authority(event.target.value) as EditorRecord["authority"] })} aria-invalid={controlInvalid("status")} aria-describedby={controlDescribedBy("status", "editor-status-error")}>{statuses.map((value) => <option key={value} value={value}>{value}</option>)}</select>{invalid("status") && <span id="editor-status-error" className="error" role="alert">{invalid("status")}</span>}<ValidationFindingMessages findings={findingsFor("status")} /><p>Authority: <strong>{authority(draft.status)}</strong> (derived from status)</p>
      <label htmlFor="editor-visibility">Visibility</label><select id="editor-visibility" value={draft.visibility.audience} onChange={(event) => update({ visibility: event.target.value === "warden" ? { audience: "warden", warden_only: true } : { audience: event.target.value as "players" | "shared", warden_only: false } })} aria-invalid={controlInvalid("visibility")} aria-describedby={controlDescribedBy("visibility", "editor-visibility-error")}><option value="warden">Warden only</option><option value="shared">Shared</option><option value="players">Players</option></select>{invalid("visibility") && <span id="editor-visibility-error" className="error" role="alert">{invalid("visibility")}</span>}<ValidationFindingMessages findings={findingsFor("visibility")} />
    </fieldset>
    <fieldset disabled={fieldsLocked}><legend>Fields</legend>{draft.fields.map((field, index) => <div key={field.field_id}><label htmlFor={`editor-field-${field.field_id}`}>{field.field_id}</label><input id={`editor-field-${field.field_id}`} value={String(field.value ?? "")} readOnly={!definitions.recordDefinitions[draft.record_type]?.fields.includes(field.field_id)} onChange={(event) => setField(index, { ...field, value: event.target.value })} aria-invalid={controlInvalid(`field-${field.field_id}`)} aria-describedby={controlDescribedBy(`field-${field.field_id}`, `editor-field-${field.field_id}-error`)} />{invalid(`field-${field.field_id}`) && <span id={`editor-field-${field.field_id}-error`} className="error">{invalid(`field-${field.field_id}`)}</span>}<ValidationFindingMessages findings={findingsFor(`field-${field.field_id}`)} /></div>)}</fieldset>
    <fieldset disabled={fieldsLocked}><legend>Content sections</legend>{draft.sections.map((section, index) => <div key={section.section_id}><label htmlFor={`editor-section-${section.section_id}`}>{section.section_id}</label><textarea id={`editor-section-${section.section_id}`} rows={5} readOnly={!definitions.recordDefinitions[draft.record_type]?.sections.some((item) => item.id === section.section_id)} value={section.body} onChange={(event) => setSection(index, { ...section, body: event.target.value })} aria-invalid={controlInvalid(`section-${section.section_id}`)} aria-describedby={controlDescribedBy(`section-${section.section_id}`, `editor-section-${section.section_id}-error`)} />{invalid(`section-${section.section_id}`) && <span id={`editor-section-${section.section_id}-error`} className="error" role="alert">{invalid(`section-${section.section_id}`)}</span>}<ValidationFindingMessages findings={findingsFor(`section-${section.section_id}`)} /></div>)}</fieldset>
    <fieldset disabled={fieldsLocked}><legend>Typed connections ({draft.connections.length})</legend>{draft.connections.map((connection, index) => <ConnectionEditor key={connection.connection_id} campaignId={campaignId} revision={view.head_revision} relationships={definitions.relationships} connectionStates={definitions.connectionStates} connection={connection} error={invalid(`connection-${connection.connection_id}`)} connectionFindings={findingsFor(`connection-${connection.connection_id}`)} targetFindings={findingsFor(`connection-${connection.connection_id}-target`)} relationshipFindings={findingsFor(`connection-${connection.connection_id}-relationship`)} stateFindings={findingsFor(`connection-${connection.connection_id}-state`)} contextError={invalid(`connection-${connection.connection_id}-context`)} contextFindings={findingsFor(`connection-${connection.connection_id}-context`)} onChange={(next) => update({ connections: draft.connections.map((item, itemIndex) => itemIndex === index ? next : item) })} onRemove={() => update({ connections: draft.connections.filter((_, itemIndex) => itemIndex !== index) })} />)}<button type="button" onClick={() => update({ connections: [...draft.connections, { connection_id: nextConnectionId(draft.connections), target_record_id: "", relationship: definitions.relationships[0] ?? "connected-to", state: definitions.connectionStates[0] ?? "current", context: "Describe this connection." }] })}>Add typed connection</button></fieldset>
...
function ConnectionEditor({ campaignId, revision, relationships, connectionStates, connection, error, connectionFindings, targetFindings, relationshipFindings, stateFindings, contextError, contextFindings, onChange, onRemove }: { campaignId: string; revision: RevisionRef; relationships: string[]; connectionStates: string[]; connection: EditorConnection; error?: string; connectionFindings: EditorFinding[]; targetFindings: EditorFinding[]; relationshipFindings: EditorFinding[]; stateFindings: EditorFinding[]; contextError?: string; contextFindings: EditorFinding[]; onChange: (connection: EditorConnection) => void; onRemove: () => void }) {
  const contextErrorId = `connection-context-${connection.connection_id}-error`;
  return <div className="editor-connection" role="group" aria-label={`Connection ${connection.connection_id}`} aria-describedby={findingIds(connectionFindings)}><ValidationFindingMessages findings={connectionFindings} /><RecordPicker campaignId={campaignId} revision={revision} label={`Target for ${connection.connection_id}`} triggerId={`connection-target-${connection.connection_id}`} value={connection.target_record_id} onChange={(target_record_id) => onChange({ ...connection, target_record_id })} error={error} findings={targetFindings} /><label htmlFor={`connection-relationship-${connection.connection_id}`}>Relationship</label><select id={`connection-relationship-${connection.connection_id}`} value={connection.relationship} onChange={(event) => onChange({ ...connection, relationship: event.target.value })} aria-invalid={relationshipFindings.length > 0} aria-describedby={findingIds(relationshipFindings)}>{!relationships.includes(connection.relationship) && <option value={connection.relationship} disabled>Unsupported: {connection.relationship}</option>}{relationships.map((value) => <option key={value} value={value}>{value}</option>)}</select><ValidationFindingMessages findings={relationshipFindings} /><label htmlFor={`connection-state-${connection.connection_id}`}>State</label><select id={`connection-state-${connection.connection_id}`} value={connection.state} onChange={(event) => onChange({ ...connection, state: event.target.value })} aria-invalid={stateFindings.length > 0} aria-describedby={findingIds(stateFindings)}>{!connectionStates.includes(connection.state) && <option value={connection.state} disabled>Unsupported: {connection.state}</option>}{connectionStates.map((value) => <option key={value} value={value}>{value}</option>)}</select><ValidationFindingMessages findings={stateFindings} /><label htmlFor={`connection-context-${connection.connection_id}`}>Context</label><textarea id={`connection-context-${connection.connection_id}`} rows={2} value={connection.context} onChange={(event) => onChange({ ...connection, context: event.target.value })} aria-invalid={!!contextError || contextFindings.length > 0} aria-describedby={describedBy(contextError ? contextErrorId : undefined, findingIds(contextFindings))} />{contextError && <span id={contextErrorId} className="error" role="alert">{contextError}</span>}<ValidationFindingMessages findings={contextFindings} /><button type="button" onClick={onRemove}>Remove connection {connection.connection_id}</button></div>;
}
function RemovalResolution({ campaignId, revision, impact, resolutions, setResolutions, errors, disabled }: { campaignId: string; revision: RevisionRef; impact: EditorRemovalImpact; resolutions: Array<Record<string, unknown>>; setResolutions: (value: Array<Record<string, unknown>>) => void; errors: Record<string, string>; disabled: boolean }) { return <section aria-labelledby="removal-impact-heading" className="editor-impact"><h3 id="removal-impact-heading">Removal impact and resolutions</h3><p>{impact.incoming_references.length} incoming typed connection(s) require a decision.</p>{impact.incoming_references.map((reference) => { const current = resolutions.find((item) => item.reference_id === reference.reference_id); const action = current?.action === "accept_unresolved" && !reference.permitted_unresolved ? "" : String(current?.action ?? ""); const error = errors[`resolution-${reference.reference_id}`]; const detailsId = `resolution-details-${reference.reference_id}`; const context = reference.context.length > 2000 ? `${reference.context.slice(0, 1997)}…` : reference.context; return <fieldset key={reference.reference_id} disabled={disabled}><legend>{reference.source_record_id} · {reference.relationship} · {reference.connection_id}</legend><p id={detailsId}>Reference <code>{reference.reference_id}</code> · connection <code>{reference.connection_id}</code> · state <code>{reference.state}</code>. Context: {context}</p><label htmlFor={`resolution-${reference.reference_id}`}>Resolution for {reference.reference_id}</label><select id={`resolution-${reference.reference_id}`} required value={action} aria-invalid={!!error} aria-describedby={[detailsId, error ? `resolution-${reference.reference_id}-error` : null].filter(Boolean).join(" ") || undefined} onChange={(event) => setResolutions(resolutions.map((item) => item.reference_id === reference.reference_id ? { reference_id: reference.reference_id, action: event.target.value, replacement_target_record_id: event.target.value === "redirect" ? "" : null } : item))}><option value="" disabled>Choose a resolution</option><option value="remove_reference">Remove reference</option><option value="redirect">Redirect reference</option>{reference.permitted_unresolved && <option value="accept_unresolved">Accept unresolved</option>}</select>{error && <span id={`resolution-${reference.reference_id}-error`} className="error" role="alert">{error}</span>}{action === "redirect" && <RecordPicker campaignId={campaignId} revision={revision} label={`Replacement target for ${reference.reference_id}`} value={String(current?.replacement_target_record_id ?? "")} onChange={(target) => setResolutions(resolutions.map((item) => item.reference_id === reference.reference_id ? { ...item, replacement_target_record_id: target } : item))} />}</fieldset>; })}</section>; }
