...
export interface GenerationView extends ContractEnvelope<"generation_view"> { generation_id: PublicId; campaign_id: PublicId; source_revision: PublicId; draft_authority: "draft"; status: "pending" | "complete" | "failed" | "cancelled"; sources: ReadonlyArray<GenerationSource>; source_set_digest: Digest; last_sequence: number; terminal_content: string | null; terminal_content_digest: Digest | null; }
...
// Retained transport-neutral Atlas family. The HTTP slice does not render the full Atlas.
export interface CampaignAtlas extends ContractEnvelope<"campaign_atlas"> { campaign: { campaign_id: PublicId; name: string; adapter_id: "mothership" }; viewed_revision: RevisionRef; head_revision: PublicId; entities: ReadonlyArray<{ entity_id: PublicId; entity_type: PublicId; name: string; authority: Exclude<Authority, "table_fact"> }>; connections: ReadonlyArray<{ connection_id: PublicId; from_entity_id: PublicId; to_entity_id: PublicId; label: string; direction: "directed" | "undirected" }>; backlinks: ReadonlyArray<{ source_entity_id: PublicId; target_entity_id: PublicId; connection_id: PublicId }>; history: ReadonlyArray<{ event_id: PublicId; revision_id: PublicId; authority: Exclude<Authority, "table_fact">; title: string }>; comparison: { from_revision: PublicId; to_revision: PublicId; changes: ReadonlyArray<{ subject_id: PublicId; change_type: "added" | "removed" | "changed" | "authority_changed" }> }; }
