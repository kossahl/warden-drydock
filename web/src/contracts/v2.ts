...
export type OperationName = "campaign_create" | "session_start" | "session_capture" | "session_end" | "live_start" | "live_takeover" | "live_capture" | "live_end" | "proposal_correct" | "proposal_reject" | "proposal_approve" | "provider_configure" | "provider_consent" | "projection_rebuild" | "backup_create";
...
export type LiveCaptureType = "confirmed_fact" | "unresolved_question";
export type LiveSessionMode = "active" | "ended_review_pending" | "ended";
export interface LiveEvent { event_id: PublicId; event_type: LiveCaptureType; device_id: PublicId; operation_id: PublicId; device_order: number; payload_digest: Digest; base_revision: PublicId; grounding_eligible: boolean; record_id: string | null; }
export interface LiveAcknowledgement { device_id: PublicId; operation_id: PublicId; payload_digest: Digest; outcome: "accepted" | "exact_replay" | "digest_conflict"; }
export interface LiveEndBarrier { end_device_id: PublicId; end_operation_id: PublicId; required_operation_ids: ReadonlyArray<{ device_id: PublicId; operation_id: PublicId }>; acknowledged_operation_ids: ReadonlyArray<{ device_id: PublicId; operation_id: PublicId }>; ready_for_proposal: boolean; }
export interface LiveSessionView extends ContractEnvelope<"live_session_view"> { session_id: PublicId; campaign_id: PublicId; base_revision: PublicId; reported_head_revision: PublicId; workflow_version: number; controller: { epoch: number; controller_id: PublicId; mode: "controller" }; mode: LiveSessionMode; events: ReadonlyArray<LiveEvent>; acknowledgements: ReadonlyArray<LiveAcknowledgement>; end_barrier: LiveEndBarrier | null; overlay: { overlay_id: PublicId; authority: "non_canon"; base_revision: PublicId; confirmed_fact_ids: ReadonlyArray<PublicId>; question_ids: ReadonlyArray<PublicId>; }; }
export interface LiveCaptureResult extends ContractEnvelope<"live_capture_result"> { outcome: "accepted" | "exact_replay" | "digest_conflict"; campaign_id: PublicId; session_id: PublicId; event_id: PublicId; device_id: PublicId; operation_id: PublicId; session: LiveSessionView; }
...
export interface AtlasOverviewStory { session_id: PublicId; title: string; summary: string; summary_authority: "author_written" | "table_fact"; played_at: string; source_revision: AtlasRevisionRef; }
export type AtlasNextActionKind = "resume_session" | "resume_conversation" | "continue_campaign" | "start_session";
export interface AtlasOverview extends ContractEnvelope<"atlas_overview"> { binding: AtlasBinding; campaign_name: string; adapter_id: "mothership"; record_count: number; edge_occurrence_count: number; approved_revision_count: number; campaign_created_at: string; display_timezone: string; player_count: number; player_count_policy: "active_non_archived"; last_played_at: string | null; latest_story: AtlasOverviewStory | null; next_action: { kind: AtlasNextActionKind; label: string }; conversation: { conversation_id: PublicId; state: "new" | "resumable" }; facets: AtlasFacets; }
...
export interface AtlasRecordMatchPart { text: string; matched: boolean; }
export interface AtlasRecordMatch { field: string; label: string; parts: ReadonlyArray<AtlasRecordMatchPart>; }
export interface AtlasRecordLibraryItem extends AtlasRecordSummary { matches: ReadonlyArray<AtlasRecordMatch>; }
export interface AtlasRecordLibraryResult extends ContractEnvelope<"atlas_record_library_result"> { binding: AtlasBinding; normalized_query: string; filters: AtlasFilters; limit: number; sort: "record_id"; total: number; items: ReadonlyArray<AtlasRecordLibraryItem>; facets: AtlasFacets; next_cursor: string | null; previous_cursor: string | null; }
