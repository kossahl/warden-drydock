# P3-CONTRACTS v2: Hosted domain contracts

## Status

Implemented; awaiting pull-request review.
...
- **Owned files:** `docs/contracts/hosted/**`, `tests/hosted/contracts/**`, and
...
The `pyproject.toml` ownership is limited to the dev-only conforming JSON Schema
validator; it does not authorize a runtime dependency. The implementer is not
authorized to change core, CLI, standalone, database,
...
family and publish a single versioned index that names every schema, fixture,
and mandatory semantic-invariant specification. Cross-field rules that JSON
Schema cannot express remain part of the indexed contract and require
categorized executable golden vectors. Use a conforming Draft 2020-12 validator
from the dev dependency set for schema and meta-schema checks.
