...
class SliceApplication:
    ...
    def _editor_transition_changes(before: dict | None, after: dict | None, change_id: str) -> tuple[list[dict], list[dict]]:
        ...
        visibility = []
        if before is None:
            if after["authority"] in {"canon", "revealed"}:
                authority.append({"change_id": change_id, "record_id": after["record_id"], "from": "absent", "to": after["authority"], "explicit_in_diff": True, "warden_approval_required": True})
    ...
    def _editor_proposal(self, campaign_id: str, revision_id: str, payload: dict, kind: str, record_id: str | None = None, *, operation_name: str | None = None, proposal_id_override: str | None = None, correction_of: dict | None = None) -> tuple[int, dict]:
        ...
        source_changes = [{
            ...
            "subject_record_id": change.subject_id,
            "change_type": change.change_kind.value,
            "before_source": source_before[change.change_id],
            "after_source": None if change.change_kind is ChangeKind.DELETE else change.replacement,
