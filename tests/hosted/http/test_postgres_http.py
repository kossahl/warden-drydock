...
class PostgresHTTPReceiptIntegrationTests(unittest.TestCase):
    ...
    def test_editor_publication_crash_before_postgres_finalization_recovers_once(self) -> None:
        suffix = uuid.uuid4().hex[:16]
        campaign_id = f"campaign_editor_crash_{suffix}"
        revision = None
        proposal_id = None
        keys = [f"idem_editor_crash_{name}_{suffix}" for name in ("campaign", "edit", "approve")]
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            campaign_payload = bind({
                "contract_name": "campaign_create_request", "contract_version": 2,
                "operation_request": operation("campaign_create", f"request_campaign_{suffix}", keys[0]),
                "input": {"campaign_id": campaign_id, "campaign_name": "Editor PostgreSQL Crash", "adapter_id": "mothership"},
            })
            revision = app.create_campaign(campaign_payload)[1]["head_revision"]
            record_id = "record-editor-crash"
            candidate = {
                "record_id": record_id, "record_type": "npc",
            ...
            with mock.patch.object(
                app.workflow, "finalize_editor_publication", side_effect=SystemExit("publication crash")
            ):
                with self.assertRaises(SystemExit):
                    app.editor_proposal_approve(proposal_id, 1, approval)
            self.assertEqual(2, len(app.revisions.store.inventory()))
            self.assertEqual("approving", app.proposal_repository.get(proposal_id, 1).status.value)
            self.assertEqual(revision, app.workflow.head(campaign_id))
            self.assertEqual(
                "pending",
                app.workflow.matching_intents(app._id("token", proposal_id, 1))[0].status.value,
            )
            self.assertEqual(
                0,
                sum(row[4] == "published" for row in app.proposal_repository.audit(proposal_id)),
            )
            ...
            recovered = restarted.proposal_repository.get(proposal_id, 1)
            self.assertEqual("published", recovered.status.value)
            self.assertEqual(recovered.published_revision_id, restarted.workflow.head(campaign_id))
            self.assertEqual(
                "finalized",
                restarted.workflow.matching_intents(
                    restarted._id("token", proposal_id, 1)
                )[0].status.value,
            )
            self.assertEqual(
                proposal["editor_workflow_version"] + 1,
                restarted._editor_version(campaign_id),
            )
            published_audit = tuple(
                row for row in restarted.proposal_repository.audit(proposal_id)
                if row[4] == "published"
            )
            self.assertEqual(1, len(published_audit))

            status, replay = restarted.editor_proposal_approve(
                proposal_id, 1, approval,
            )
            self.assertEqual((200, "published"), (status, replay["outcome"]))
            self.assertEqual(published_audit, tuple(
                row for row in restarted.proposal_repository.audit(proposal_id)
                if row[4] == "published"
            ))
